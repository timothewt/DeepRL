...
class FCNetMultiHeadMasked(nn.Module):
	...
	def __init__(self, config: dict[str: Any]):
		...
		"""
		super(FCNetMultiHeadMasked, self).__init__()
	...
	def forward(self, x: tensor, masks: tuple[tensor]) -> list[tensor]:
		x = self.fc(x)
		return [
			self.output_function(
				self.output_layers[i](x) + torch.clamp(torch.log(masks[i]), min=-3.4e38)
